    ...
    <div className="text-white w-full h-screen relative select-none"
      ...
      <section className='absolute pl-6 pt-4 bg-black/70 w-screen h-full'>
        ...
        <div className='pt-6'>
          ...
          <p className="mt-4 md:max-w-[700px]">{movieData.overview}</p>
          <p className="mt-2 text-gray-400">Release Date: {movieData.release_date}</p>
          <p>{movieData.origin_country}</p>
          <p>Rating: <span>{movieData.vote_average.toFixed(2)}</span></p>
         ...
         {/* { <Link href={``}>
          ...
          {/* {<p>{movieData.production_companies.map((company: ProductionCompany, index: number) =>{
